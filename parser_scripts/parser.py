...
wyniki_z_nutch = r'P:/all/'    # katalog z wynikami z nutch
...
def odszukaj_licencje(strona):
    for klucz in licencje.iterkeys():
        if klucz in strona.get('text', '').lower():
            strona['license'] = licencje[klucz]
            print strona['id'], licencje[klucz]
            return 1
    for klucz in licencje_njdz.iterkeys():
        if ('licen' in strona.get('text', '').lower()) and (klucz in strona.get('text', '').lower()):
            strona['license'] = licencje_njdz[klucz]
            print strona['id'], licencje_njdz[klucz]
            return 1


for nazwa_pliku in os.listdir(wyniki_z_nutch):
    adres = '{0}{1}'.format(wyniki_z_nutch, nazwa_pliku)
    nutch = file(adres)
    print 'miele', adres
    ...
    for linia in nutch:
        ...
        if re.match(tagi[0], linia):
            #print '-------------nowa strona-------------'
            if not odszukaj_licencje(dane[-1]):
    ...
    adres = r'../parsed_results/{0}_parsed_filtered.txt'.format(nazwa_pliku)
    print 'zapisuje w', adres
    plik = file(adres, 'w')
